...
function getKrogerPrice(searchItem) {
  ...
  var settings2 = {
    ...
    crossDomain: true,
    url:
      "https://api.kroger.com/v1/products?filter.brand=Kroger&filter.term=" +
      searchItem.split(" ").join("%20") +
...
function displayMeals(mealSearch) {
  var requestUrl =
    "https://www.themealdb.com/api/json/v1/1/filter.php?i=" + mealSearch;
  fetch(requestUrl)
          ...
          for (var i = 0; i < data.meals.length; i++) {
            ...
            if (data.meals[i].strMeal.length > 17) {
              mealName.textContent =
                data.meals[i].strMeal.substring(0, 17) + "..."; // Disaplyed only 17 characters in textcontent
...
function mealRecipeModal(meal) {
  ...
  for (var e = 1; e < 21; e++) {
    ...
    ingredientsListItems.innerHTML =
      meal["strMeasure" + e] + "   " + meal["strIngredient" + e];
  }
